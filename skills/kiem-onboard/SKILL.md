...
(respecting `.gitignore`) for markdown files that have YAML frontmatter or a
clear H1 — any `*.md` anywhere in the repo, not just a fixed set of folders.
A path/filename keyword match only refines the inferred type below; it is not
required for inclusion:
- `plan` → `plan` · `requirements` or `brainstorm` → `brainstorm` ·
  `solutions` → `solution` · `decision`/`decisions` → `decision` ·
  `review`/`reviews` → `review` · no keyword hit (e.g. `docs/specs/*`) → `doc`
  (default, not skipped). Multiple keywords match the same path (e.g.
  `docs/plan-and-decision-review.md`) → first match wins, in the order listed
  above.
- `AGENTS.md`, `README*`, and `CLAUDE.md` are never import/delete candidates
  here even if their path would otherwise match — they're living project
  config, already handled (checklist mining only) in step 4.
